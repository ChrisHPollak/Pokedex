...
class Pokemon {
	...
	private var _nextEvoultionName: String!
	...
	var nextEvolutionName: String {
		...
		if _nextEvoultionName == nil {
			_nextEvoultionName = ""
		}
		return _nextEvoultionName
	}
		...
		Alamofire.request(pokemonURL).responseJSON { (response) in
			...
			if let dict = response.result.value as? Dictionary<String, AnyObject> {
				...
				if let descriptionArray = dict["descriptions"] as? [Dictionary<String, AnyObject>], descriptionArray.count > 0 {
					...
					if let url = descriptionArray[0]["resource_uri"] as? String {
				...
				if let evolutions = dict["evolutions"] as? [Dictionary<String, AnyObject>], evolutions.count > 0 {
					...
					if let nextEvolution = evolutions[0]["to"] as? String {
						...
						if nextEvolution.range(of: "mega") == nil {
							...
							self._nextEvoultionName = nextEvolution
